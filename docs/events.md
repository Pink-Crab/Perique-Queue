...
There are 3 types of event which can be extended within your Application. These allow you to dispatch custom events to the [Event Queue](queue-service.md) to be handled by your Application.
...
As the name implies, these events are triggered as soon as possible by the [Event Queue](queue-service.md). They are not delayed nor recurring.
...
**Optional Properties**
* `$is_unique` - This denotes if the event is unique. If set to true, the event will only be added to the queue if it does not already exist. This is useful for events which are triggered multiple times but should only be processed once. Defaults to `false`.
...
It is possible to define an event which will not be triggered until a defined time. This is useful for events which need to be triggered at a later date, or after a certain amount of time.
...
This event will be triggered 1 day after it is dispatched to the [Event Queue](queue-service.md).
...
> The interval is defined by an integer representing the number of seconds.

**Required Properties**
* `$hook` - The WP Hook/Action which will be triggered when the event is processed.
* `$group` - The hooks group is used to identify the plugin or theme which the event belongs to.
* `$data` - The data to be passed to the event.
...
The event will be triggered 1 day after it is dispatched to the [Event Queue](queue-service.md), and then every 1 day after that.


# Dispatching Events

This can then be dispatched to the [Event Queue](queue-service.md) by either injecting the `Queue_Service` into your Controller or by using the `Queue::dispatch()` helper function.
...
It is possible to create ad hoc events, which are not defined as a concrete class, but instead using an anonymous class.
